...
import '../style/webtrain.css';
import 'webpack-jquery-ui';

import '../libs/gojs/go-debug.js';
...
function uploadFile(e, obj) {
  console.log('%c func uploadFile', 'background:#5089b0;color:#fff;')
  switch (nodeBody.testType) {
    case 'batch':
      // for(var i in formFiles){
      //     formData.append(i, formFiles[i]);
      // }

      // formData在 btnOpt 中已经赋值
      ajax({
        url: "/upload_batch_pics/",
        data: formData,
        method: 'POST',
        progress: true,
        success: function (data) {
    ...
    case 'classes':
      {
        ...
        ajax({
          ...
          success: function (data) {
            ...
            if (data.less_pics_folder.length > 0) {
              msg = data.less_pics_folder.join(', ')
              alert(msg + "  几个品规的图片数量较少，推荐50张以上")
            };
          }
        })
      }
  ...
  console.log('%c func uploadFile END', 'background:#5089b0;color:#fff;')
}
...
function switchImg(opt) {
  return function () {
    ...
    (checkPic.imgIndex < 0) && (checkPic.imgIndex = 0);
    (checkPic.imgIndex >= imgs.length) && (checkPic.imgIndex = imgs.length - 1);

    checkPic.currentImg = imgs[checkPic.imgIndex];
    checkPic.src = 'check_imgs/' + imgs[checkPic.imgIndex];
  }
}
btnPre.onclick = switchImg('-'); btnNext.onclick = switchImg('+');

/** 
 * 处理错误的检查图：生成批次检查图后，如果检查突出错则添加到错误列表中
 * @method handleErrImg
 */
function handleErrImg() {
  switch (this.name) {
    case 'do':
      if (!checkPic.currentImg) return;

      errImgs.push(checkPic.currentImg);
      var node = doc.createElement('div');
      node.className = 'err';
      node.innerText = checkPic.currentImg;
      errsList.appendChild(node);
      break;

    case 'undo':
      errImgs.pop();
      errsList.removeChild(errsList.children[errsList.children.length - 1]);
      break;
...
btnFalse.onclick = handleErrImg;
undo.onclick = handleErrImg;
 ...
 */
function exec() {
  // 判断 nodeBody.testType 类型，在index.js中的 addModelChangedListener中绑定。
  if (!nodeBody.testType) return;

  var obj = myDiagram.findNodesByExample({
    'category': 'result'
  }).first();

  obj.findObject('result').text = '正在训练';
  loading.style.display = 'block';
  switch (nodeBody.testType) {
    case 'classes':
      ajax({
        method: 'POST',
        url: '/start_classes_train/',
        success: function (data) {
          var data = JSON.parse(data);
          var date = new Date().toLocaleString()
          loading.style.display = 'none';
          if (data.msg === 'failed') {
            obj.findObject('result').text = '训练失败';
            alert('训练失败:' + date);
          } else {
            successTrain = data.success_train;

            obj.findObject('result').text = '训练完成';
            alert(data.success_train.join(',')+'训练完成:' + date);
          }
        }
      })
      break;
 ...
 */
function coverModel(){
  console.log('%c'+'coverModel', 'background:green; color:#fff;');
  var formData = new FormData(doc.getElementsByClassName('j-cover-form')[0]);
  loading.style.display = 'block';
  ajax({
    url: "/replace_model/",
    data: formData,
  ...
  console.log('%c'+'coverModel END', 'background:green; color:#fff;');
}
btnCover.onclick = coverModel;
...
function restartServer(){
  loading.style.display = 'block';
  ajax({
    url: '/restart_vision_grpc_server/',
    method: 'POST',
    success: function(data){
      loading.style.display = 'none';
      var data = JSON.parse(data);
 ...
 */
function usrDefTest(){
  if( nodeBody.contains(doc.getElementsByClassName('usr-def')[0])){return;}

  var form = doc.createElement('form');
  form.className = 'usr-def';

  htmlstr = '<div class="input">起始日期: <input class="j-start-date" type="date" name="first_date" /></div>\
    <div class="input">终止日期: <input class="j-deadline" type="date" name="second_date"/> </div>\
    <button type="button" class="confirm">更新</button> <button type="button" class="cancle">取消</button>';

  form.innerHTML = htmlstr;

  var btnCancle = form.getElementsByClassName('cancle')[0];
  var btnConfirm = form.getElementsByClassName('confirm')[0];

  btnCancle.onclick = function(){ nodeBody.removeChild(form) }

  btnConfirm.onclick = function(){
    loading.style.display = 'block';
    var formData = new FormData(document.getElementsByClassName("usr-def")[0]);
    ajax({
      url: "/proc_old_data/",
      data: formData,
      method: 'POST',
      success: function (data) {
        ...
        var data = JSON.parse(data);
        var msg;
        console.log('usrDefTest');
        console.log(data);
        btnCancle.onclick();
      }
    })  
  }
  nodeBody.appendChild(form);
}
btnTestPic.onclick = usrDefTest;
